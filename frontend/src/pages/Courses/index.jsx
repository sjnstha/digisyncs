    ...
    <div style={{ fontFamily: "var(--font-site)" }}>
      <section className="py-14" style={{ background: "var(--color-primary)" }}>
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-blue-100">{t("courses.page_subtitle", "JLPT preparation from N5 to N1")}</p>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          {loading ? <LoadingSpinner /> : (
            <div className="space-y-6">
              {courses?.map((course) => {
                const colors = LEVEL_COLORS[course.level_code] || { bg: "#f8f9ff", text: "#1e2d7d", border: "#2d3eb0" };
                return (
                  <div key={course.id}
                    className="rounded-xl p-6 flex flex-col sm:flex-row gap-6 items-start"
                    style={{ background: colors.bg, border: `1px solid ${colors.border}` }}>
                    <div className="flex-shrink-0 text-center">
                      <div className="text-3xl font-black" style={{ color: colors.text }}>
                        {course.level_code}
                      </div>
                    ...
                    <div className="flex-1">
                      <h2 className="font-bold text-lg mb-2" style={{ color: colors.text }}>
                        {tl(course, "title")}
                      </h2>
                      {tl(course, "description") && (
                        <p className="text-sm leading-relaxed mb-3" style={{ color: colors.text, opacity: 0.8 }}>
                          {tl(course, "description")}
                        </p>
                      )}
                      <div className="flex flex-wrap gap-4 text-sm">
                        {course.duration_weeks && (
                          <span style={{ color: colors.text }}>⏱ {course.duration_weeks} weeks</span>
                        )}
                        {course.schedule && (
                          <span style={{ color: colors.text }}>📅 {course.schedule}</span>
                        )}
                        {course.price && (
